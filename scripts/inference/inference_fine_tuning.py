...
# === CONFIGURAÇÕES ===
CSV_PATH = os.path.expanduser("~/Proj_NPA/nlp_genAI_projectV3/data/processed/edmunds_test.csv") ## já está alterado
N_FEW_SHOT = 3
REGION = "eu-west-1"
#ENDPOINT_NAME = "g3-llama32-ft-2025-07-24-17-21-18" ## alterar

ENDPOINT_NAME = "g3-llama32-ft-2025-07-25-16-00-40" ## alterar
...
# === CARREGAR DADOS ===
df = pd.read_csv(CSV_PATH, delimiter=';')  # OU sep=';'
...
Action:"""
    ...
    print(f"[{idx+1}/{len(df)}] Sentiment (Z/F): {sentiment_zero} / {sentiment_few} | Action (Z/F): {action_zero} / {action_few}")


    results.append({
        "Review_Title": row.get("Review_Title", ""), # <---- alterado aqui
        "Review": review,
...
df_resultados.to_csv("~/Proj_NPA/nlp_genAI_projectV3/data/processed/resultados_finetuning.csv", index=False)
print("✅ Resultados gravados em 'resultados_finetuning.csv'")
