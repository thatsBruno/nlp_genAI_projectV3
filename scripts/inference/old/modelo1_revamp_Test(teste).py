...
def build_zeroshot_prompt(example):
    ...
    return extract_prompt_parts(example["input"])
...
def main(task="sentiment", n_shots=3):
    ...
    step = f"Examples Len: "
    log_col(func_n+step, len(examples))
    # Few-shot
    ...
    step = f"Examples size before zero shIt"
    log_col(func_n+step, len(examples))
    step = f"n_shots +1"
    log_col(func_n+step, n_shots+1)
    prompt_zs = build_zeroshot_prompt(examples[n_shots+1])
    step = f"\n===== ZERO-SHOT PROMPT =====\n"
    log_col(func_n+step, prompt_zs)
    out_zs = query_endpoint(prompt_zs)
    step = f"\n--- Zero-shot output ---"
    log_col(func_n+step, out_zs)
